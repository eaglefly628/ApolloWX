...
export interface CanvasRendererOptions {
  ...
  assets?: AssetManager;
  /**
   * 可选的平台 canvas。无 DOM 的环境（微信小游戏 wx.createCanvas() / OffscreenCanvas）通过它直接
   * 注入画布；提供时不再 document.createElement，也不依赖 init(container)。浏览器留空即按原行为
   * 自建 canvas 并挂到 container。
   */
...
export class CanvasRenderer implements RendererBackend {
  ...
  init(container?: HTMLElement): void {
    // 平台 canvas（微信 wx.createCanvas()）优先；否则浏览器自建。
    ...
    // 微信小游戏：用 wx.getSystemInfoSync().pixelRatio（无 window）。
    this.dpr = (typeof window !== 'undefined' && window.devicePixelRatio) || 1;
    canvas.width = Math.round(this.logicalW * this.dpr);
    canvas.height = Math.round(this.logicalH * this.dpr);
    // wx canvas 无 style；仅 DOM canvas 钉 CSS 逻辑尺寸。
    const style = (canvas as { style?: CSSStyleDeclaration }).style;
    if (style) {
      style.width = `${this.logicalW}px`;
      style.height = `${this.logicalH}px`;
    }
    // 仅在浏览器自建 canvas 时挂到 container；平台注入的 canvas 已是全屏主画布。
  ...
  destroy(): void {
    // wx canvas 无 remove()；仅 DOM canvas 从文档移除。
    this.canvas?.remove?.();
    this.canvas = null;
    this.ctx = null;
  }
